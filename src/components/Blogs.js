...
function Blogs() {
    ...
    return (
        ...
        <div className='flex flex-col w-full h-screen items-center my-5'>
            {blogs.map(blogs => (
                <BlogCard image={blogs.image} title={blogs.title} description={blogs.description} littleDescription={blogs.littleDescription} author={blogs.author} date={blogs.date} />
            ))}
        </div>
    )
}
